...
errors = np.zeros((ratio_num, epoch_step))
    ...
    for epoch in range(epoch_step * epoch_step_num):
        ...
        if (epoch + 1) % epoch_step_num == 0:
            ...
            with torch.no_grad():
                ...
                errors[i][(epoch + 1) // epoch_step_num - 1] = error_avg
...
x = [epoch_step_num * (i + 1) for i in range(epoch_step)]
with open(f'./data/{filename}_charge_ratio_expect_ratio_{input_spectrum_num}_{ratio_num}.csv', 'w') as csv_file:
    csv_file.write('Epochs, ')
    for i in range(len(x)):
        csv_file.write(f'{x[i]}, ')
    csv_file.write('\n')

    for i in range(ratio_num):
        csv_file.write(f'Ratio 1/{2**(i + 1)} error (%), ')
        for j in range(epoch_step):
            csv_file.write(f'{errors[i][j]}, ')
        csv_file.write('\n')
