...
def density_unnormalized():
    ...
    for ac in ac_data:
        a_index, c_index = int(ac[0] / grid_size), int(ac[1] / grid_size)
    ...
    ax.plot_surface(A, C, Density)
    ax.contour(A, C, Density, zdir='z', offset=-10, cmap='coolwarm')
    ax.set_xlabel('a')
    ax.set_ylabel('c')
    ax.set_zlabel('Density')
    ax.set_zlim(-10, max_density)
...
def density_inverse():
    ...
    for ac in ac_data:
        a_index, c_index = int((1.0 / ac[0]) / grid_size), int((1.0 / ac[1]) / grid_size)
        Density[c_index, a_index] += 1.0
        if Density[c_index, a_index] > max_density:
            max_density = Density[c_index, a_index]
    ...
    with open(f'./data/landscape_inverse_ac_density_grid_{grid_size}_maxima_distance_{min_maxima_distance}.csv', 'w', newline='') as csvfile:
        ...
        for i in range(len(local_max_a)):
            csvwriter.writerow([1 / local_max_a[i], 1 / local_max_c[i], local_max_density[i]])
    ...
    ax.plot(local_max_a, local_max_c, 'rx')
    ax.set_xlabel('1/a')
    ax.set_ylabel('1/c')
...
def density_log():
    ...
    grid_max_a = float(int(max_log_24a + 24))
    grid_min_c = float(int(min_log_48c))
    grid_max_c = float(int(max_log_48c + 48))
    ...
    for ac in ac_data:
        a_index, c_index = int(np.log(24 * ac[0]) / grid_size), int(np.log(48 * ac[1]) / grid_size)
        Density[c_index, a_index] += 1.0
        if Density[c_index, a_index] > max_density:
            max_density = Density[c_index, a_index]
    ...
    ax.plot_surface(A, C, np.log(Density + 1), cmap=cm.coolwarm)
    ax.contour(A, C, np.log(Density + 1), zdir='z', offset=-10, cmap='coolwarm')
