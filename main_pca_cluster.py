...
superpotential_refined = [[0 for _ in range(len(terms) * 3)] for _ in range(len(superpotentials))]
for i in range(len(superpotentials)):
    for j in range(len(superpotentials[i])):
...
pca.fit(superpotential_refined)
superpotential_reduced = pca.transform(superpotential_refined)
print(superpotential_reduced.shape)
superpotential_refined = superpotential_reduced.tolist()

for i in range(len(superpotential_refined)):
    superpotential_refined[i].append(float(data[i + 1][a_index]))
    superpotential_refined[i].append(float(data[i + 1][c_index]))
